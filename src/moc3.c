...
#define psm__model_check_range(begin_arr, count_arr, i, max) \
   ...
   */
#define psm__check_key_combo(obj_src, kf_total, obj_count) \
  for (psm__i32 _i = 0; _i < (obj_count); _i++) { \
    ...
    PSM__FAIL(!psm__valid_range(_kb, _mc, (kf_total)), \
        PSM__ERR_FILE_CORRUPT, \
        "%s[%d] keyform_offset=%d max_blend=%d total=%d", \
  ...
  for (psm__i32 i = 0; i < cnt->parts; i++) {
    ...
    psm__model_check_range(src->part_src.keyform_offset,
        src->part_src.key_count, i, cnt->part_kf);
  ...
  for (psm__i32 i = 0; i < cnt->warps; i++) {
    psm__model_check_index(src->warp_src.binding_idx,
        i, cnt->bindings);
    psm__model_check_range(src->warp_src.keyform_offset,
        src->warp_src.key_count, i, cnt->warp_kf);
  ...
  for (psm__i32 i = 0; i < cnt->warps; i++) {
    psm__i32 begin = src->warp_src.keyform_offset[i];
    psm__i32 count = src->warp_src.key_count[i];
    psm__i32 vc = src->warp_src.vertex_count[i];
    for (psm__i32 j = 0; j < count; j++) {
      psm__i32 pb = src->warp_key_src.key_pos_offset[begin + j];
  ...
  for (psm__i32 i = 0; i < cnt->rotations; i++) {
    ...
    psm__model_check_range(src->rotation_src.keyform_offset,
        src->rotation_src.key_count, i, cnt->rotation_kf);
  }
  ...
  for (psm__i32 i = 0; i < cnt->art_meshes; i++) {
    ...
    psm__model_check_range(src->art_mesh_src.keyform_offset,
        src->art_mesh_src.key_count, i, cnt->art_mesh_kf);
    psm__model_check_index_or_neg1(
    ...
    PSM__FAIL(src->art_mesh_src.vertex_count[i] < 0 ||
        src->art_mesh_src.uv_begin[i] < 0 ||
        (psm__u32)src->art_mesh_src.uv_begin[i] +
            2u * (psm__u32)src->art_mesh_src.vertex_count[i]
            > (psm__u32)cnt->uvs, PSM__ERR_FILE_CORRUPT,
        "art_mesh[%d]: UV [%d, +%d*2) oob (max %d)",
        i, src->art_mesh_src.uv_begin[i],
        src->art_mesh_src.vertex_count[i], cnt->uvs);
    psm__model_check_range(src->art_mesh_src.indices_begin,
        src->art_mesh_src.indices_count, i, cnt->indices);
    psm__model_check_range(src->art_mesh_src.mask_begin,
        src->art_mesh_src.mask_count, i, cnt->masks);
  }
  ...
  for (psm__i32 i = 0; i < cnt->art_mesh_kf; i++) {
    psm__model_check_index(src->art_mesh_key_src.key_pos_offset,
        i, cnt->kf_pos);
  }
  ...
  for (psm__i32 i = 0; i < cnt->glues; i++) {
    ...
    psm__model_check_range(src->glue_src.keyform_offset,
        src->glue_src.key_count, i, cnt->glue_kf);
    ...
    psm__model_check_range(src->glue_src.info_begin,
        src->glue_src.info_count, i, cnt->glue_info);
  }

  /* Glue position indices */
  if (src->glue_src.info_begin && src->glue_src.info_count &&
    ...
    for (psm__i32 i = 0; i < cnt->glues; i++) {
      ...
      psm__i32 ib = src->glue_src.info_begin[i];
      psm__i32 ic = src->glue_src.info_count[i];
  ...
  for (psm__i32 i = 0; i < cnt->draw_groups; i++) {
    psm__model_check_range(src->draw_group_src.obj_begin,
        src->draw_group_src.obj_count, i, cnt->draw_items);
  }
  ...
  for (psm__i32 i = 0; i < cnt->warps; i++) {
    psm__model_check_range(src->warp_src.key_color_offset,
        src->warp_src.key_count, i, cnt->kf_mul_colors);
  }

  /* Rotation deformer color indices */
  for (psm__i32 i = 0; i < cnt->rotations; i++) {
    psm__model_check_range(src->rotation_src.key_color_offset,
        src->rotation_src.key_count, i, cnt->kf_mul_colors);
  }

  /* Art mesh color indices */
  for (psm__i32 i = 0; i < cnt->art_meshes; i++) {
    psm__model_check_range(src->art_mesh_src.key_color_offset,
        src->art_mesh_src.key_count, i, cnt->kf_mul_colors);
  }
  ...
  for (psm__i32 i = 0;
    ...
    psm__model_check_range(src->bs_warp_src.bs_binding_begin,
        src->bs_warp_src.bs_binding_count, i, cnt->blend_bindings);
  }
  ...
  for (psm__i32 i = 0;
    ...
    psm__model_check_index(src->bs_art_mesh_src.target_idx,
        i, cnt->art_meshes);
    psm__model_check_range(src->bs_art_mesh_src.bs_binding_begin,
        src->bs_art_mesh_src.bs_binding_count, i, cnt->blend_bindings);
  }
  ...
  for (psm__i32 i = 0;
    ...
    psm__model_check_index(src->blend_constraint_src.parameter_idx,
        i, cnt->parameters);
    psm__model_check_range(src->blend_constraint_src.value_begin,
        src->blend_constraint_src.value_count, i, cnt->bs_constraint_vals);
  }
  ...
  for (psm__i32 i = 0; i < cnt->bs_parts; i++) {
    ...
    psm__model_check_range(src->bs_part_src.bs_binding_begin,
        src->bs_part_src.bs_binding_count, i, cnt->blend_bindings);
  }
  ...
  for (psm__i32 i = 0;
    ...
    psm__model_check_range(src->bs_rotation_src.bs_binding_begin,
        src->bs_rotation_src.bs_binding_count, i, cnt->blend_bindings);
  }

  /* Blend shape glue sources */
  for (psm__i32 i = 0; i < cnt->bs_glues; i++) {
    psm__model_check_index(src->bs_glue_src.target_idx, i, cnt->glues);
    psm__model_check_range(src->bs_glue_src.bs_binding_begin,
        src->bs_glue_src.bs_binding_count, i, cnt->blend_bindings);
  }
  ...
  for (psm__i32 i = 0; i < cnt->offscreens; i++) {
    ...
    psm__model_check_range(src->offscreen_src.mask_begin,
        src->offscreen_src.mask_count, i, cnt->masks);
  }
  ...
  for (psm__i32 i = 0;
    ...
    psm__model_check_index(src->bs_offscreen_src.target_idx,
        i, cnt->offscreens);
    psm__model_check_range(src->bs_offscreen_src.bs_binding_begin,
        src->bs_offscreen_src.bs_binding_count, i, cnt->blend_bindings);
  }
...
psm__revive_moc_in_place(struct psm__moc3_data **moc, psm__u8 *p, psm_size n)
{
  ...
  if (count > 0) {
    psm__i32 *mb = src->art_mesh_src.mask_begin;
    psm__i32 *mc = src->art_mesh_src.mask_count;
...
skip_mask_processing:
  ...
  if (count > 0 && src->art_mesh_src.id && src->art_mesh_src.id_runtime) {
    for (psm__i32 i = 0; i < count; i++) {
      ...
      if (src->uv_src.xy && src->art_mesh_src.uv_begin) {
        psm__i32 ub = src->art_mesh_src.uv_begin[i];
        if (psm__check_idx(ub, cnt->uvs))
          src->art_mesh_src.uv_runtime[i] = &src->uv_src.xy[ub];
      }
      if (src->indices_src.index && src->art_mesh_src.indices_begin) {
        psm__i32 pb = src->art_mesh_src.indices_begin[i];
        if (psm__check_idx(pb, cnt->indices))
          src->art_mesh_src.position_idx_runtime[i] =
              &src->indices_src.index[pb];
      }
      if (src->mask_src.art_mesh_idx && src->art_mesh_src.mask_begin) {
        psm__i32 mb2 = src->art_mesh_src.mask_begin[i];
  ...
  if (ver >= csmMocVersion_53) {
    ...
    if (count > 0 && src->offscreen_src.drawable_mask_runtime &&
        src->mask_src.art_mesh_idx && src->offscreen_src.mask_begin) {
      for (psm__i32 i = 0; i < count; i++) {
        psm__i32 ob = src->offscreen_src.mask_begin[i];
  ...
  if (src->canvas_info && (src->canvas_info->flag &
    ...
    psm__i32 *idx_begin = src->art_mesh_src.indices_begin;
    psm__i32 *idx_cnt = src->art_mesh_src.indices_count;
    ...
    psm__i32 *uv_begin = src->art_mesh_src.uv_begin;

    if (uv_xy && vert_cnt && uv_begin) {
      for (psm__i32 i = 0; i < count; i++) {
        psm__i32 vc = vert_cnt[i];
        psm__i32 ub = uv_begin[i];
